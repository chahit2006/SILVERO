...
### Filter Specification — Part 1 of 7 done (`FILTER_SPEC_IMPLEMENTATION.md` Build order)

**Part 1, Attributes Manager — ✅ built 2026-08-22.** Filter options are now the client's to manage; changing them is no longer a code change.
- `FilterHeading` / `FilterOption` models — migration `20260822090000_filter_attributes`. Headings stay fixed in code (`lib/attributes.ts`'s `FILTER_HEADINGS`), per the spec's "adding a heading is a developer job"; all six keys exist — `finish`, `stone`, `stone_color`, `design_style`, `occasion`, `collection`.
- **`/admin/attributes`** — per-heading add / rename / delete / reorder (native HTML5 drag plus ↑/↓ buttons, which are the real control on touch and keyboard — no drag library, CLAUDE.md #1). Each option shows its live product count.
- **Product form** — Finish/Stone/Occasion are dropdowns fed by `FilterOption` instead of free-text inputs. A stored value that isn't a registered option any more still renders, flagged "(not in Filter Attributes)", rather than being silently rewritten. "— None —" now actually clears a field (the API maps `""` → `null`); the old free-text inputs couldn't.
- **PLP hide-when-empty** — `getPlpFilterGroups()` renders only options at least one non-archived product *in that view* carries, in admin order, and drops a whole section when nothing survives. Scoped to the route's base filters, never the shopper's own selections — same rule as `getPriceBounds()`, so a checkbox can't vanish under the click that set it. The static `MATERIAL_OPTIONS`/`STONE_OPTIONS`/`OCCASION_OPTIONS` lists in `lib/filter-options.ts` are gone.
- **`prisma/seed-attributes.ts`** — idempotent; seeds starter options *and* backfills every distinct `material`/`stone`/`occasion` value existing products already carry. That backfill is not optional: the PLP now only offers registered options, so an unregistered legacy value would otherwise disappear from the sidebar on deploy. Runs standalone (`npm run prisma:seed-attributes`) and as part of `prisma/seed.ts`.
- **Two deliberate guardrails, not in the spec:** deleting an option that products still use is refused with the count (deleting would strand those products on a value no filter offers, with no way back to the exact string); renaming one rewrites the products carrying it, in the same transaction.
- ⚠️ **Not verified against a live database** — no Postgres was reachable in the build environment (`P1000` auth failure against `localhost:5432`). Typecheck and lint are clean and the handwritten migration SQL was verified byte-for-byte against `prisma migrate diff`, but the migration has not been applied, the seed has not run, and no admin/PLP round trip has been exercised. Do that before the client review.

**Parts 2–7 not started.** Two client decisions still block work — the price filter conflict (buckets vs. the shipped slider) and the sort dropdown (keep live vs. roll back to Hindi-labelled inactive framework). Neither blocks Part 1, which is why it went first.
