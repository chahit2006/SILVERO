...
- **`account/(dashboard)/` route group (added 2026-08-08, not in the original tree above).** `login/`, `register/`, and `forgot-password/` must stay reachable while logged out, but every other `/account/*` page needs a server-side auth gate (`requireUser()`). A shared `app/account/layout.tsx` would gate the login page itself, so the gated pages live in `app/account/(dashboard)/` with their own `layout.tsx` calling `requireUser()`, while login/register/forgot-password stay as plain siblings under `app/account/`. URLs are unaffected (`(dashboard)` adds no segment) — `account/(dashboard)/orders/page.tsx` still resolves to `/account/orders`.
- **PRD.md §2's sitemap lists `/register` and `/forgot-password` without the `/account` prefix**, but this file (the structural source of truth) nests them under `account/`. Built per this file's nesting — `/account/register`, `/account/forgot-password` — flagging the sitemap doc as inconsistent rather than picking silently.
